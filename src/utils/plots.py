...
def plot_price_curve(S_range: List[float], K: float, r: float, sigma: float, T: float, 
                    option_type: OptionType) -> plt.Figure:
    """
    ...
    ax.plot(S_range, prices, 'b-', linewidth=2, label=f'{option_type.title()} Option')
    ax.axvline(x=K, color='r', linestyle='--', alpha=0.7, label=f'Strike (K={K})')
    ax.set_xlabel('Stock Price (S)')
    ax.set_ylabel('Option Price')
    ax.set_title(f'{option_type.title()} Option Price vs Stock Price\n'
...
def plot_price_heatmap(S_values: List[float], vol_values: List[float], K: float, r: float, T: float, 
    ...
    # Create heatmap
    im = ax.imshow(price_grid, cmap='viridis', aspect='auto', 
    ...
    ax.set_title(f'{option_type.title()} Option Price Heatmap\n'
                 f'K={K}, r={r:.3f}, T={T:.3f}')
    
    # Add strike line
    ax.axvline(x=K, color='r', linestyle='--', alpha=0.8, label=f'Strike (K={K})')
    ax.legend()
