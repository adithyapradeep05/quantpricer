...
from .normals import normal_pdf, safe_sqrt
...
def _price_function(S: float, K: float, T: float, r: float, sigma: float, option_type: OptionType, target_price: float) -> float:
    ...
    try:
        ...
        raise ValueError(f"Error in price function: {e}")
...
def _vega_function(S: float, K: float, T: float, r: float, sigma: float) -> float:
    ...
    try:
        ...
        raise ValueError(f"Error in vega function: {e}")
...
def implied_vol_from_price(price: float, S: float, K: float, r: float, T: float, 
    ...
    try:
        ...
        raise ValueError(f"No-arbitrage violation: {e}")
    ...
    try:
        ...
        raise ValueError(f"Error evaluating price function: {e}")
    ...
    if f_low * f_high > 0:
        ...
        try:
            ...
            raise ValueError(f"Error evaluating price function at low volatility: {e}")
        ...
        if f_low * f_high > 0:
            ...
            try:
                ...
                raise ValueError(f"Error evaluating price function at high volatility: {e}")
    ...
    for i in range(max_iter):
        ...
        try:
            ...
            raise ValueError(f"Error in bisection iteration {i}: {e}")
    ...
    for i in range(max_iter):
        try:
            ...
            raise ValueError(f"Error in Newton-Raphson iteration {i}: {e}")
        ...
        try:
            ...
            raise ValueError(f"Error calculating vega in iteration {i}: {e}")
