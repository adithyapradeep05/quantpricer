...
class HeatmapRequest(BaseModel):
    ...
    def validate_S_values(cls, v):
        if not v:
            raise ValueError('S_values cannot be empty')
        if len(v) > 200:
            raise ValueError('Too many S_values (max: 200)')
        if any(s <= 0 for s in v):
            raise ValueError('All S_values must be positive')
        if any(s > 10000 for s in v):
            raise ValueError('All S_values must be <= 10,000')
...
class GreeksCurveResponse(BaseModel):
    S_values: List[float] = Field(..., description="Stock price values")
    delta: List[float] = Field(..., description="Delta values")
    gamma: List[float] = Field(..., description="Gamma values")
    vega: List[float] = Field(..., description="Vega values")
    theta: List[float] = Field(..., description="Theta values (per year)")
    rho: List[float] = Field(..., description="Rho values")
